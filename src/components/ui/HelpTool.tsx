...
const HelpTool: React.FC<HelpToolProps> = ({ className = '', userBag, onToolUsed, sendHelpTool, onUserBagUpdate, onError, onShowToolEffect, disabled = false, canUseTool }) => {
    ...
    const renderHelpToolButton = (
        ...
        if (type === 'hint' && isOwned) {
            ...
            icon = (
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-white opacity-50">
        ...
        return (
            <div key={index} className="relative">
                    ...
                    content={
                        isOwned ? (
                            <div className="text-center text-xs ">
                                ...
                                <div className="mt-1 opacity-50">{quantity} lượt</div>
                            </div>
                        ) : (
                            `${title}`
                        )
                ...
                >
                    <button 
                        className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95 disabled:cursor-not-allowed"
                        style={{
                            background: gradient,
                            boxShadow: isOwned 
