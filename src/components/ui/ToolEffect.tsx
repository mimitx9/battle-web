...
interface ToolEffectProps {
    toolType: string;
    isVisible: boolean;
    onAnimationComplete?: () => void;
}

const ToolEffect: React.FC<ToolEffectProps> = ({ toolType, isVisible, onAnimationComplete }) => {
    ...
    useEffect(() => {
        if (isVisible) {
            ...
            // Chuyển sang phase stay sau 300ms
            const stayTimer = setTimeout(() => {
                setAnimationPhase('stay');
            }, 300);

            // Chuyển sang phase exit sau 800ms
            const exitTimer = setTimeout(() => {
                setAnimationPhase('exit');
            }, 800);

            // Hoàn thành animation sau 1200ms
    ...
    // Mapping tool type sang thông tin hiển thị
    const getToolInfo = (type: string) => {
        switch (type) {
            case 'battleHint':
                return {
                    gradient: 'linear-gradient(135deg, #FF8C00 0%, #FFD406 50%, #FF8C00 100%)',
                    icon: (
            ...
            case 'battleSnow':
                return {
                    gradient: 'linear-gradient(135deg, #0A0158 0%, #644EFD 50%, #0A0158 100%)',
                    icon: (
                        <svg width="80" height="80" viewBox="0 0 43 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            case 'battleBlockTop1':
                return {
                    gradient: 'linear-gradient(135deg, #250000 0%, #E05B00 50%, #250000 100%)',
                    icon: (
                        <svg width="80" height="80" viewBox="0 0 73 73" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            case 'battleBlockBehind':
                return {
                    gradient: 'linear-gradient(135deg, #250000 0%, #DC00E0 50%, #250000 100%)',
                    icon: (
                        <svg width="80" height="80" viewBox="0 0 73 73" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="36.5" cy="36.5" r="31.5" stroke="white" strokeWidth="10"/>
                        </svg>
            ...
            default:
                return {
                    gradient: 'linear-gradient(135deg, #666 0%, #999 50%, #666 100%)',
                    icon: null,
                    title: 'Tool',
                    description: 'Đã sử dụng tool'
                };
        }
    };
    ...
    const getAnimationClass = () => {
        switch (animationPhase) {
            ...
            case 'exit':
                return 'animate-ping scale-125 opacity-0';
            default:
                return '';
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
            {/* Background overlay với gradient */}
            <div 
                className={`absolute inset-0 transition-all duration-500 ${
                    animationPhase === 'enter' ? 'opacity-20' : 
                    animationPhase === 'stay' ? 'opacity-30' : 'opacity-0'
                }`}
                style={{
                    background: `radial-gradient(circle, ${toolInfo.gradient.split(',')[0].replace('linear-gradient(135deg, ', '').replace(' 0%', '')} 0%, transparent 70%)`
                }}
            />
            
            {/* Main effect container */}
            <div className={`relative transition-all duration-500 ${getAnimationClass()}`}>
                {/* Outer glow ring */}
                <div 
                    className="absolute inset-0 rounded-full animate-spin"
                    style={{
                        background: `conic-gradient(from 0deg, transparent, ${toolInfo.gradient.split(',')[1].trim()}, transparent)`,
                        filter: 'blur(8px)',
                        transform: 'scale(1.2)'
                    }}
                />
                ...
                <div 
                    ...
                    style={{
                        background: toolInfo.gradient,
                        boxShadow: `0 0 40px ${toolInfo.gradient.split(',')[1].trim()}, inset 0 0 20px rgba(255,255,255,0.3)`
                    }}
                >
                    {/* Icon */}
                ...
                <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 text-center">
                    <div 
                        ...
                        style={{
                            textShadow: `0 0 10px ${toolInfo.gradient.split(',')[1].trim()}`,
