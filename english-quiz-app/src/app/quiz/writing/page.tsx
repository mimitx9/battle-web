...
const WritingQuizPage: React.FC = () => {
    ...
    const [currentSkill, setCurrentSkill] = useState('Writing');
    ...
    const writingSections = allSections.filter(section => section.sectionType === 'Writing');
    ...
    const convertToSubmitFormat = useCallback((quizSections: QuizSection[], userAnswers: {[key: string]: string}, userId: number): QuizSubmitData => {
        ...
        const resolveCurrentQuizId = () => {
            ...
            const sectionFromState = writingSections[currentSectionIndex];
            ...
            // 3) Nếu vẫn chưa có, lấy phần đầu tiên của kỹ năng Writing (nếu tồn tại)
            const firstWriting = quizSections.find(s => s.sectionType === 'Writing');
            if (firstWriting) return firstWriting.quizId;
    ...
    const switchToNextSkill = useCallback(async () => {
        ...
        try {
            ...
            const attemptIdToUse = attempt.attemptId || attempt.id;
            if (attemptIdToUse && attemptIdToUse !== 0) {
                // Đặt isCurrent sang phần đầu tiên của Speaking
                const nextType = 'Speaking' as const;
                ...
                console.log('✅ Progress saved before switching to Speaking');
            }

            // Navigate to next skill
            router.push('/quiz/speaking');
    ...
    useEffect(() => {
        ...
        const initQuiz = async () => {
            try {
                ...
                if (currentSection && currentSection.sectionType !== 'Writing') {
                    ...
                    const thisIdx = order.indexOf('Writing');
                ...
                // Set current section index for writing (within writing sections)
                const writingSections = attemptData.quizSections.filter(s => s.sectionType === 'Writing');
                const currentWritingSection = writingSections.find(s => s.isCurrent);
                if (currentWritingSection) {
                    const writingIndex = writingSections.findIndex(s => s.quizId === currentWritingSection.quizId);
                    if (writingIndex >= 0) {
                        setCurrentSectionIndex(writingIndex);
                    }
                } else if (writingSections.length > 0) {
                    // Fallback to first writing section if no current section found
                ...
                if (attemptData.startedAt) {
                    ...
                    const timeLeft = calculateTimeLeft(attemptData.startedAt, 'Writing');
    ...
    useEffect(() => {
        if (timeLeft > 0) {
            ...
            // Khi hết thời gian Writing, tự động chuyển sang Speaking
            console.log('⏰ Writing time expired, switching to Speaking');
            switchToNextSkill();
        }
    }, [timeLeft, attempt, switchToNextSkill]);

    // Get current section directly from writing sections
    const currentSection = writingSections[currentSectionIndex];
    ...
    const nextSection = () => {
        if (currentSectionIndex < writingSections.length - 1) {
            const nextSectionIndex = currentSectionIndex + 1;
            setCurrentSectionIndex(nextSectionIndex);
        } else {
            // This is the last section of Writing, show skill switch modal
    ...
    const switchToSection = (sectionIndex: number) => {
        ...
        if (targetSection.sectionType === currentSkillType) {
            // Find the index within writing sections
            const writingIndex = writingSections.findIndex(s => s.quizId === targetSection.quizId);
            if (writingIndex >= 0) {
                setCurrentSectionIndex(writingIndex);
            }
        }
    ...
    const toggleMenu = () => {
        ...
        return (
                        ...
                        {currentSection?.medias?.some(media => media.type === 'audio') && currentAudioUrl && (
                            <div className="mb-4">
                                ...
                                <AudioPlayer
                                    ...
                                    onPlayClick={() => { const a = audioRef.current; if (a) a.play(); }}
                                />
                            </div>
                        )}
                    </div>
                                                ...
                                                <div className="flex justify-end mb-2">
                                                    <div className="text-sm">
                                                        ...
                                                        <span className="text-blue-600 font-semibold">
                                                            {countWords(answers[question.questionId.toString()] || '')}
                                                        </span>
                                                    </div>
                                                </div>
                                                <textarea
            ...
            <LoadingOverlay
                ...
                subMessage="AI đang chấm điểm bài viết, vui lòng kiên nhẫn chờ đợi"
            />
        </div>
    );
};
